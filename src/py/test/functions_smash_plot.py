...
def save_figure(plot,figname="myfigure",xsize=8,ysize=6,transparent=False,dpi=80,xlim=[None,None],ylim=[None,None]):
...
def plot_image(matrice=np.zeros(shape=(2,2)),bbox=None,title="",xlabel="",ylabel="",zlabel="",vmin=None,vmax=None,mask=None,cmap="viridis",figname="",title_font_size=22):
    """
    ...
    cmap: string of the colormap, default = 'viridis' : https://matplotlib.org/stable/users/explain/colors/colormaps.html
    ...
    im=ax.imshow(matrice,extent=extend,vmin=vmin,vmax=vmax,cmap=cmap)
    ax.axes.set_xlabel(xlabel, fontsize=18)
    ax.axes.set_ylabel(ylabel, fontsize=18)
...
def plot_model_params_and_states(model,variables,title=None,fstates=False):
...
def plot_dist_wjreg(res_assim):
    ...
    for key,values in res_assim.items():
        ...
        if len(lcurve.keys())>0:
            
            # ~ print(key,lcurve.keys())
            color="black"
            point_type="."
            markersize=6
    ...
    ax1.scatter(np.array(xn),np.array(yn), color=color,marker=point_type)
    ax1.set_xlabel("log(wjreg) - Lcurve method");
    ax1.set_ylabel("log(wjreg) - Fast method");
    
    min_val=min(min(xn),min(yn))
    max_val=max(max(xn),max(yn))
    x_aff=np.arange(min_val,max_val+1)
    y_aff=np.arange(min_val,max_val+1)
    ax1.plot(x_aff,y_aff, color="red",marker=None,markersize=markersize)
    
    # ~ frac_wjreg=np.zeros(shape=np.array(x).shape)
    frac_wjreg=np.array(y)/np.array(x)
    # ~ frac_wjreg=yn/xn
    # ~ print(frac_wjreg.shape)
    mean_wjreg=np.mean(frac_wjreg)
    
    ax2.set_ylabel("(wjreg approx) / (wjreg opt)");
    ax2.boxplot(frac_wjreg,0,"")
    
    fig.suptitle(f"Ratio (wjreg approx) / (wjreg opt), mean value = {mean_wjreg}")
