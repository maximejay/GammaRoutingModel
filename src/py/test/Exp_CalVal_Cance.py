...
model_gamma=gamma.smashplug.ConfigureGammaWithSmash(smash_model,dt=900,vmin=0.1,vmax=10.,mode_discretization_step=0.1,spreading_discretization_step=0.1,ponderation_regul=0.0,velocity_computation="qm3",varying_spread=1,spreading_uniform=1,criteria='nse',ponderation_cost=10000.,pdt_start_optim=1600)
...
GammaGriddedObservation=gamma.smashplug.SmashDataVectorsToGrid(smash_model.input_data.qobs,model_gamma,smash_dt=smash_model.setup.dt)

#compute the initial cost
initial_cost=model_gamma.cost_function(GammaGriddedObservation,model_gamma.routing_results.discharges)

#Get the control vector
ControlVector=gamma.smashplug.VectorizeModelParameters(smash_model,model_gamma,control_parameters_list=["cp","cft","hydraulics_coefficient","spreading"])
...
ControlVector.update({"bounds":{"cp":[0.1,1000.],"cft":[0.1,1000.],"hydraulics_coefficient":[0.3,5.],"spreading":[0.5,3.]}})

#get the inflows
GammaInflows=gamma.smashplug.GetGammaInflowFromSmash(smash_model,dt=900)

#Compute the gradient
gradient=gamma.smashplug.ComputeModelGradients(ControlVector, smash_model, model_gamma, GammaInflows, GammaGriddedObservation)
...
BestControlVector,optimized_smash_model,optimized_gamma_model=gamma.smashplug.OptimizeCoupledModel(
        smash_model,
        model_gamma,
        GammaGriddedObservation,
        control_parameters_list=["cp","cft","hydraulics_coefficient","spreading"],
        bounds={"cp":[0.1,1000.],"cft":[0.1,1000.],"hydraulics_coefficient":[0.3,5.],"spreading":[0.5,3.]},
        maxiter=15,
        tol=0.00001,
...
GammaGriddedObservation=gamma.smashplug.SmashDataVectorsToGrid(optimized_smash_model.input_data.qobs,optimized_gamma_model,smash_dt=optimized_smash_model.setup.dt)
#compute the final cost
model_gamma.cost_function(GammaGriddedObservation,optimized_gamma_model.routing_results.discharges)
...
ax.plot(abcisses["ts_gamma"],model_gamma.routing_results.discharges[:,model_gamma.routing_mesh.controlled_nodes[0]-1],label="SMASH-grg-Gamma, Qsim Initial",lw=2) #nodes are in Fortran index: in python array we need -1: how to fix that ?

ax.plot(abcisses["ts_gamma"],optimized_gamma_model.routing_results.discharges[:,model_gamma.routing_mesh.controlled_nodes[0]-1],label="SMASH-grg-Gamma, Qsim Optimal",lw=2) #nodes are in Fortran index: in python array we need -1: how to fix that ?

ax.plot(abcisses["ts_smash"],model_smash_gr_calibrated.output.qsim[0,:],label="Reference SMASH-gra, Qsim Optimal",lw=2)

ax.plot(abcisses["ts_gamma"],GammaGriddedObservation[:,model_gamma.routing_mesh.controlled_nodes[0]-1],label="Observed discharges (OBS)",color='0',lw=2)
...
functions_smash_plot.save_figure(plot,figname="ExpCalVal.pdf",xlim=[0,1500],xsize=24,ysize=10)
functions_smash_plot.save_figure(plot,figname="ExpCalVal_zoom1.pdf",xlim=[550,725],ylim=[0,250],xsize=12,ysize=10)
functions_smash_plot.save_figure(plot,figname="ExpCalVal_zoom2.pdf",xlim=[1150,1400],ylim=[0,350],xsize=12,ysize=10)

#convert gamma vector parameters to gridded data
Grid_Hydraulic_Coef=gamma.smashplug.GammaVectorsToSmashGrid(optimized_gamma_model.routing_parameters.hydraulics_coefficient,smash_model,model_gamma)

Spreading=gamma.smashplug.GammaVectorsToSmashGrid(optimized_gamma_model.routing_parameters.spreading,smash_model,model_gamma)

#plot that parameters
functions_smash_plot.plot_image(Grid_Hydraulic_Coef,figname="ExpCalVal_CoefHydraulics.pdf",mask=smash_model.mesh.active_cell,title="Hydraulic coefficients",title_font_size=14)

functions_smash_plot.plot_image(Spreading,figname="ExpCalVal_Spreading.pdf",mask=smash_model.mesh.active_cell,title="Spreading coefficients",title_font_size=14)

functions_smash_plot.plot_image(optimized_smash_model.parameters.cp,figname="ExpCalVal_Cp.pdf",mask=smash_model.mesh.active_cell,title="Capacities of the production reservoir",title_font_size=14)

functions_smash_plot.plot_image(optimized_smash_model.parameters.cft,figname="ExpCalVal_cft.pdf",mask=smash_model.mesh.active_cell,title="Capacities of the transfert reservoir",title_font_size=14)


#get observed discharges:
GammaGriddedObservation=gamma.smashplug.SmashDataVectorsToGrid(model_smash_gr_calibrated.input_data.qobs,optimized_gamma_model,smash_dt=model_smash_gr_calibrated.setup.dt)
...
nse_grg=functions_smash_stats.nse(GammaGriddedObservation[:,optimized_gamma_model.routing_mesh.controlled_nodes[0]-1],optimized_gamma_model.routing_results.discharges[:,optimized_gamma_model.routing_mesh.controlled_nodes[0]-1],lacuna=-99.)
...
model_gamma_validation.routing_parameters_change(hydraulics_coefficient=optimized_gamma_model.routing_parameters.hydraulics_coefficient.copy(),spreading=optimized_gamma_model.routing_parameters.spreading.copy())
...
functions_smash_plot.save_figure(plot,figname="ExpCalVal_validation_period.pdf",xlim=[0,5000],xsize=24,ysize=10)
functions_smash_plot.save_figure(plot,figname="ExpCalVal_validation_period_zoom1.pdf",xlim=[150,250],ylim=[0,50],xsize=12,ysize=10)
functions_smash_plot.save_figure(plot,figname="ExpCalVal_validation_period_zoom2.pdf",xlim=[2760,2860],ylim=[0,100],xsize=12,ysize=10)

#get observed discharges:
GammaGriddedObservation=gamma.smashplug.SmashDataVectorsToGrid(model_smash_grg_validation.input_data.qobs,model_gamma_validation,smash_dt=model_smash_grg_validation.setup.dt)

#Compute the NSE
nse_gra=functions_smash_stats.nse(model_smash_gra_validation.input_data.qobs[0,:],model_smash_gra_validation.output.qsim[0,:],lacuna=-99.)
