...
def VectorIndexesOfSmashGauges(smash_model):
    ...
    index_gauge_pos=MatrixCoordsToVectorIndexes(smash_model.mesh.gauge_pos[:,0],smash_model.mesh.gauge_pos[:,1],ncol)
        ...
        for col in range(ncol):
            ...
            if smash_model.mesh.active_cell[row,col]>0:
                ...
                for up_nd in range(nb_upstream_nodes):
                    ...
                    if (col_imd >= 0) and (col_imd < ncol) and (row_imd >= 0) and (row_imd < nrow) :
...
def SmashMeshToGammaMesh(smash_model,model_gamma):
    ...
    model_gamma.routing_mesh_init(nb_nodes=nb_nodes,nb_upstream_nodes=nb_upstream_nodes) #initialise the gamma mesh
    ...
    model_gamma.routing_mesh.upstream_to_downstream_nodes=np.argsort(VectorMesh1D["FlowAcc"])+1
    model_gamma.routing_mesh.nodes_linker=nodes_linker
    model_gamma.routing_mesh.controlled_nodes=gauge_pos+1
    model_gamma.routing_mesh.surface=(smash_model.mesh.dx)**2. / 1000.0**2.  #!km²
    model_gamma.routing_mesh.dx=smash_model.mesh.dx
...
def ConfigureGammaWithSmash(smash_model,dt=None,**kwargs):
    ...
    if smash_model.setup.structure!="gr-g":
        print("Warnings: setting smash_model.setup.structure to gr-g")
        smash_model.setup.structure="gr-g"
...
def GetGammaInflowFromSmash(smash_model,dt=None):
    ...
    inflows_domain=np.zeros(shape=(smash_model.setup._ntime_step,nb_nodes))
    
    #inflow dans le domaine
    i=0
    for row in range(nrow):
        ...
        for col in range(ncol):
            ...
            if smash_model.mesh.active_cell[row,col]>0:
                ...
                inflows_domain[:,i]=inflows_smash[row,col,:]
                i=i+1
    ...
    if dt<smash_model.setup.dt:
        ...
        for ts in range(smash_model.setup._ntime_step-1):
            ...
            for itfrac in range(frac_dt):
                ...
                interpolated_inflows[itg,:]=(inflows_domain[ts+1,:]-inflows_domain[ts,:])*(tg-ts)/(ts+1-ts) + inflows_domain[ts,:]
    ...
    if dt<smash_model.setup.dt:
        ...
        for ts in range(smash_model.setup._ntime_step-1):
            ...
            for itfrac in range(frac_dt):
                ...
                interpolated_observations[itg,:]=np.transpose((observations[:,ts+1]-observations[:,ts])*(tg-ts)/(ts+1-ts) + observations[:,ts])
    ...
    if new_dt < dt:
        ...
        for ts in range(data.shape[0]-1):
            ...
            for itfrac in range(frac_dt):
                ...
                new_data[i_nt]=(data[ts+1]-data[ts])*(i_t-ts)/(ts+1-ts) + data[ts]
...
def SmashDataVectorsToGrid(observations,model_gamma,smash_dt):
    ...
    gridded_observation=np.zeros(shape=(model_gamma.routing_setup.npdt,model_gamma.routing_mesh.nb_nodes))
    ...
    for i in range(model_gamma.routing_mesh.nb_nodes):
        ...
        if (k>0) and (k<=model_gamma.routing_mesh.nb_nodes):
            ...
            gridded_observation[:,k]=FitDataToNewDt(observations[pos_observation,:],smash_dt,model_gamma.routing_setup.dt)
...
def GriddedDataToSmashVectors(gridded_observation,model_gamma,smash_dt):
    ...
    observations=np.zeros(shape=(nb_gauge,int(gridded_observation.shape[0]*model_gamma.routing_setup.dt/smash_dt)))
    ...
    for i in range(model_gamma.routing_mesh.nb_nodes):
        ...
        if (k>0) and (k<=model_gamma.routing_mesh.nb_nodes):
            ...
            observations[pos_observation,:]=FitDataToNewDt(gridded_observation[:,k],model_gamma.routing_setup.dt,smash_dt)
            pos_observation=pos_observation+1
...
def GetCorrespondingTimeStepRange(smash_model,model_gamma):
    ...
    xgamma=np.arange(1/frac_dt,smash_model.setup._ntime_step+1/frac_dt,1/frac_dt)
...
def VectorizeModelParameters(smash_model,model_gamma,control_parameters_list=list()):
    ...
    if len(control_parameters_list) == 0:
        ...
        control_parameters_list.append(smash.core._constant.STRUCTURE_PARAMETERS[smash_model.setup.structure])
        ...
        for ctrl_var in control_parameters_list:
            ...
            if not ( hasattr(smash_model.parameters,ctrl_var) or hasattr(smash_model.states,ctrl_var) or hasattr(model_gamma.routing_parameters,ctrl_var) ):
    ...
    for ctrl_var in control_parameters_list:
        ...
        if hasattr(smash_model.parameters,ctrl_var):
            ...
            param_smash=getattr(smash_model.parameters,ctrl_var)
        ...
        if hasattr(smash_model.states,ctrl_var):
            ...
            states_smash=getattr(smash_model.states,ctrl_var)
    ...
    for ctrl_var in control_parameters_list:
        ...
        if hasattr(model_gamma.routing_parameters,ctrl_var):
            ...
            if ctrl_var == "hydraulics_coefficient":
                ...
                LinearizedParameters[k:k+nb_nodes]=model_gamma.routing_parameters.hydraulics_coefficient[:]
                k=k+nb_nodes
            
            if ctrl_var == "spreading" :
                
                LinearizedParameters[k:k+nb_nodes]=model_gamma.routing_parameters.spreading[:]
                k=k+nb_nodes
...
def SetVectorizedModelParameters(control_vector,smash_model,model_gamma):
    ...
    if not ( ("X" in control_vector) and ("ParamList" in control_vector) and ("NbNodes" in control_vector) ):
        print("Error: the control vector is an incomplete dictionary. You must provide a control vector build with the function VectorizeModelParameters")
        raise ValueError("Error: the control vector is an incomplete dictionary. You must provide a control vector build with the function VectorizeModelParameters")
        ...
        if hasattr(model_gamma.routing_parameters,ctrl_var):
            ...
            if ctrl_var == "hydraulics_coefficient":
                ...
                if model_gamma.routing_setup.hydraulics_coef_uniform==1:
                    model_gamma.routing_parameters.hydraulics_coefficient[:]=control_vector['X'][k]
                else:
                    model_gamma.routing_parameters.hydraulics_coefficient[:]=control_vector['X'][k:k+nb_nodes]
            ...
            if ctrl_var == "spreading":
                ...
                if model_gamma.routing_setup.spreading_uniform==1:
                    ...
                    model_gamma.routing_parameters.spreading[:]=control_vector['X'][k:k+nb_nodes]
                
                k=k+nb_nodes
    


def ComputeModelGradients(control_vector,smash_model,model_gamma,interpolated_inflows,observations,set_param=False,ScaleGammaGradients=True,ScaleGradientsByBounds=True):
    ...
    Grad_dCOST_dROUTINGPARAMETERS=np.zeros(shape=(2,model_gamma.routing_mesh.nb_nodes),order='F',dtype="float32")
     
    print("call gamma.routing_gamma_forward_adjoint_b: Gradients of COST with respect to varying inputs ROUTING_PARAMETERS")
    gamma.routing_gamma_forward_adjoint_b(model_gamma.routing_setup,model_gamma.routing_mesh,model_gamma.routing_parameters,interpolated_inflows,observations,model_gamma.routing_states,model_gamma.routing_results,Grad_dCOST_dROUTINGPARAMETERS)
    
    
    #Gradients of COST with respect to inflows (interpolated_inflows)
    Grad_dCOST_dINFLOWS=np.zeros(shape=(model_gamma.routing_setup.npdt,model_gamma.routing_mesh.nb_nodes),order='F',dtype="float32")
    
    print("call gamma.routing_gamma_forward_adjoint_b0: Gradients of COST with respect to varying input INFLOWS (interpolated_inflows)")
    gamma.routing_gamma_forward_adjoint_b0(model_gamma.routing_setup,model_gamma.routing_mesh,model_gamma.routing_parameters,interpolated_inflows,observations,model_gamma.routing_states,model_gamma.routing_results,Grad_dCOST_dINFLOWS)
    ...
    Grad_dQSIMDOMAIN_dPARAMETERS=smash.solver._mwd_parameters.ParametersDT(smash_model.mesh)
    Grad_dQSIMDOMAIN_dSTATES=smash.solver._mwd_states.StatesDT(smash_model.mesh)
    cost=0.
    
    print("call smash.solver._mw_forward.wrapped_forward_b0: Gradients of OUTPUT.QSIM_DOMAIN with respect to varying inputs PARAMETERS and STATES")
    smash.solver._mw_forward.wrapped_forward_b0(smash_model.setup, smash_model.mesh, smash_model.input_data, smash_model.parameters, Grad_dQSIMDOMAIN_dPARAMETERS, smash_model.states, Grad_dQSIMDOMAIN_dSTATES, smash_model.output, cost)
    ...
    LinearizedGradientsForSmash=np.zeros(model_gamma.routing_mesh.nb_nodes*nb_param_smash)
            ...
            for sub_row in range(smash_model.mesh.nrow):
                ...
                for sub_col in range(smash_model.mesh.ncol):
                    ...
                    if smash_model.mesh.active_cell[sub_row,sub_col]>0:
                        ...
                        LinearizedGradientsForSmash[k]=MatrixGradients[sub_row,sub_col]*Int_Grad_dCOST_dINFLOWS[i]
                        k=k+1
                        i=i+1
    ...
    if ScaleGammaGradients and model_gamma.routing_setup.hydraulics_coef_uniform==0:
        Grad_dCOST_dROUTINGPARAMETERS[0,:]=Grad_dCOST_dROUTINGPARAMETERS[0,:]/model_gamma.routing_mesh.cumulated_surface[:]
    
    if ScaleGammaGradients and model_gamma.routing_setup.spreading_uniform==0:
        Grad_dCOST_dROUTINGPARAMETERS[1,:]=Grad_dCOST_dROUTINGPARAMETERS[1,:]/model_gamma.routing_mesh.cumulated_surface[:]
    ...
    OutputsGradients=np.zeros(len(LinearizedGradientsForSmash)+NbControledGammaParameter*control_vector["NbNodes"])
    ...
    for ctrl_var in control_vector["ParamList"]:
        ...
        if hasattr(model_gamma.routing_parameters,ctrl_var):
            ...
            if (ctrl_var == "hydraulics_coefficient"):
                ...
                if model_gamma.routing_setup.hydraulics_coef_uniform==1:
                    OutputsGradients[position:position+control_vector["NbNodes"]]=Grad_dCOST_dROUTINGPARAMETERS[0,:]/float(control_vector["NbNodes"])
                else:
                    OutputsGradients[position:position+control_vector["NbNodes"]]=Grad_dCOST_dROUTINGPARAMETERS[0,:]
            ...
            if (ctrl_var == "spreading"):
                ...
                if model_gamma.routing_setup.spreading_uniform==1:
                    OutputsGradients[position:position+control_vector["NbNodes"]]=Grad_dCOST_dROUTINGPARAMETERS[1,:]/float(control_vector["NbNodes"])
                else:
                    OutputsGradients[position:position+control_vector["NbNodes"]]=Grad_dCOST_dROUTINGPARAMETERS[1,:]
    ...
    if ("bounds" in control_vector) and (ScaleGradientsByBounds==True):
        ...
        for param in control_vector["ParamList"]:
            ...
            amplitude=control_vector["bounds"][param][1]-control_vector["bounds"][param][0]
            ...
            if amplitude > 0:
                OutputsGradients[position:position+control_vector["NbNodes"]]=OutputsGradients[position:position+control_vector["NbNodes"]]/amplitude
...
def RunCoupledModel(smash_model,model_gamma):
    ...
    print("Run of the Smash model with structure gr-g")
    smash_model.run(inplace=True)
    
    print(f"Getting the interpolated Smash outflow at time-step {model_gamma.routing_setup.dt}")
    interpolated_inflows=GetGammaInflowFromSmash(smash_model,dt=model_gamma.routing_setup.dt)
...
def ComputeCostAndGradients(X,control_vector,smash_model,model_gamma,observations,ScaleGammaGradients=True, ScaleGradientsByBounds=True):
    ...
    gradient=ComputeModelGradients(control_vector,smash_model,model_gamma,interpolated_inflows,observations,ScaleGammaGradients=ScaleGammaGradients,ScaleGradientsByBounds=ScaleGradientsByBounds)
    
    return cost,gradient


def OptimizeCoupledModel(smash_model,model_gamma,observations,control_parameters_list=["cp","hydraulics_coefficient","spreading"],bounds={"cp":[0.1,1000.],"hydraulics_coefficient":[0.5,10],"spreading":[1.,3.]},maxiter=10,tol=0.00001,ScaleGammaGradients=True,ScaleGradientsByBounds=True,inplace=False):
    ...
    if (observations.shape != optimized_gamma.routing_results.discharges.shape):
        raise ValueError(f"Observations vectors has not the same shape ({observations.shape}) than the outputs discharges of Gamma ({optimized_gamma.routing_results.discharges.shape})")
    
    #define the control vector
    ControlVector=gamma.smashplug.VectorizeModelParameters(optimized_smash,optimized_gamma,control_parameters_list=control_parameters_list)
    
    boundaries=np.zeros(shape=(len(ControlVector['X']),2))
    
    #define the boundaris
    if (len(control_parameters_list)!=len(bounds)):
        raise ValueError("Error: The number of boundaries is different than the number of controls")
    ...
    for control in control_parameters_list:
        ...
        if hasattr(optimized_smash.parameters,control):
            ...
            if not control in bounds:
                raise ValueError(f"Error: The controlled variable {control} is not in the boundaries definition")
            
            boundaries[position:position+ControlVector["NbNodes"],0]=bounds[control][0]
            boundaries[position:position+ControlVector["NbNodes"],1]=bounds[control][1]
        ...
        if hasattr(optimized_gamma.routing_parameters,control):
            ...
            if control == "hydraulics_coefficient":
                ...
                if not "hydraulics_coefficient" in bounds:
                    raise ValueError(f"Error: The controlled variable hydraulics_coefficient is not in the boundaries definition")
                
                boundaries[position:position+ControlVector["NbNodes"],0]=bounds["hydraulics_coefficient"][0]
                boundaries[position:position+ControlVector["NbNodes"],1]=bounds["hydraulics_coefficient"][1]
                position=position+ControlVector["NbNodes"]
            
            if control == "spreading":
                
                if not "spreading" in bounds:
                    raise ValueError(f"Error: The controlled variable spreading is not in the boundaries definition")
                
                boundaries[position:position+ControlVector["NbNodes"],0]=bounds["spreading"][0]
                boundaries[position:position+ControlVector["NbNodes"],1]=bounds["spreading"][1]
                position=position+ControlVector["NbNodes"]
    ...
    res=scipy.optimize.minimize(gamma.smashplug.ComputeCostAndGradients, ControlVector['X'], args=(ControlVector,optimized_smash, optimized_gamma, observations, ScaleGammaGradients, ScaleGradientsByBounds), method="L-BFGS-B", jac=True, bounds=boundaries,tol=tol,options={'disp': True, 'iprint':101, "maxiter":maxiter})
    
    #get results
    new_parameters=res.x
    #set the optimal parameter to the Smash and Gamma Model
    ControlVector['X']=new_parameters
    gamma.smashplug.SetVectorizedModelParameters(ControlVector,optimized_smash,optimized_gamma)
    
    #Run the direct coupled model Gamma o Smash-gr-g
